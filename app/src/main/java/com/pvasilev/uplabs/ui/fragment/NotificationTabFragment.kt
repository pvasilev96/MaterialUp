...
class NotificationTabFragment : BaseTabFragment() {
    override val rootScreen: String
        get() = Screens.NOTIFICATION_LIST_SCREEN

    override fun onCreate(savedInstanceState: Bundle?) {
        val scope = Toothpick.openScope(DI.NOTIFICATION_TAB_SCOPE)
    ...
    override fun createNavigator(): Navigator {
        return object : SupportAppNavigator(activity, childFragmentManager, R.id.tab_container) {
            ...
            override fun createFragment(screenKey: String, data: Any?): Fragment? =
                    when (screenKey) {
                        Screens.NOTIFICATION_LIST_SCREEN -> NotificationListFragment()
                        else -> null
                    }
        }
    }
